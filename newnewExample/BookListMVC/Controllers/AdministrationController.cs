...
namespace BookListMVC.Controllers
{
    ...
    // cause redirect to account/login if not logged-in

    [Authorize(Roles = "Admin")]
    //Role-based authorization
    // causes redirect to /account/login if not logged-in
    // cause redirect to /account/accessdenied if not authorized
    // if authorizes with role parameter is assign 2 times among themselves,
    // the logged-in user has to be in both roles
    //[Authorize(Roles = "Admin, User")] // both can access, so if the user is in admin OR user
    ...
    ManageUserRoles(List<UserRolesViewModel> model, string userId)
        ...
        public async Task<IActionResult> EditRole(EditRoleViewModel model)
        {
            ...
            {
                // we want to change RoleName
                role.Name = model.RoleName;

                // roleManager will search for given role id and
                // will update all properties (except id)
